...
public class bot {
    ...
    private static String predictDisease(J48 tree, String[] symptoms, Instances data) {
        try {
            ...
            double[] distribution = tree.distributionForInstance(data.instance(0));
                ...
                if (line.startsWith("N" + currentNode)) {
                    ...
                    if (parts.length > 1) {
                        ...
                        if (successor.startsWith("N")) {
                            // If it's a non-leaf node, extract the node number
                            return Integer.parseInt(successor.substring(1));
                        } else if (successor.startsWith("leaf")) {
                            // If it's a leaf node, return -1 to break the loop
                            return -1;
                        } else {
                            // Handle special case for nodes like '(vertigo) Paroymsal  Positional Vertigo (10.0/5.0)'
                            successor = successor.split(" ")[0];
                            if (successor.startsWith("(vertigo)")) {
                                successor = successor.substring(successor.indexOf("(") + 1, successor.indexOf(")"));
                                return Integer.parseInt(successor);
                            }
                        }
                    }
                }
